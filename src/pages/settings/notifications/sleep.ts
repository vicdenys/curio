...
@Component({
  selector: 'sleep-not-settings',
  templateUrl: 'sleep.html'
})
export class SleepNotSettingsPage {

  private loading: any;
  private time:any = '09:00';
  private timeEnabled:boolean = false;
  private notId:number = 2;
  ...
  constructor(public navCtrl: NavController, private config: Config, private auth: AuthProvider,
    ...
    this.nativeStorage.getItem('sleepNotTime')
      .then(
        data => {
          this.time = data.sleepNotTime;
          this.timeEnabled = data.sleepNotTimeEnabled;
  ...
  private scheduleNot(date){
    ...
    this.localNotifications.schedule({
       id: this.notId,
       text: 'Hey! Tijd voor een nieuwe slaaptest.',
       sound: 'file://beep.caf',
       every: 'day',
  ...
  private storeNotData(){
    this.nativeStorage.setItem('sleepNotTime', {sleepNotTimeEnabled: this.timeEnabled, sleepNotTime: this.time, scheduledDate: this.scheduledDate})
      .then(
        () => {
